...
RTreeNode* chooseLeaf(DataPoint* point, RTree* tree) {
 ...
 int index = 0;
 int minExpansion[node->numChildren];
 int least;
 ...
 // Check for tie
 int minArea = area(node->children[index]->mbr);
...
void transferNodes(RTreeNode* full, RTreeNode* vacant, int split, int horAxis) {
 printf("inside transferNodes...\n");
 printf("full->numChildren = %d\nvacant->numChildren = %d\n", full->numChildren, vacant->numChildren);
 int index = 0;
...
void splitNode(RTreeNode* node, RTreeNode* child, RTree* tree) {
 ...
 int x_cen = (node->mbr.xmin + node->mbr.xmax) / 2;
 int y_cen = (node->mbr.ymin + node->mbr.ymax) / 2;
 ...
 int x_obj = centre(temp[i]->mbr)->x;
 int y_obj = centre(temp[i]->mbr)->y;
 ...
 printf("nodes successfully allotted to corners...\n");
 ...
 if (node->parent == NULL) {
 printf("INSIDE splitNode root split\n");
...
void splitLeaf(RTreeNode* leaf, DataPoint* point, RTree* tree) {
 ...
 int x_cen = (leaf->mbr.xmin + leaf->mbr.xmax) / 2;
 int y_cen = (leaf->mbr.ymin + leaf->mbr.ymax) / 2;
 ...
 // insertNode(L1, tree);
 // insertNode(L2, tree);
 // insertNode calls splitNode if necessary

 free(leaf);

 if (parent->numChildren == MAX_CHILDREN) {
 printf("splitNode function called...\n");
